...
case ESTHER_TRG_IOCG_COUNTER:
  ...
  */
  ...
  case ESTHER_TRG_INVERT_ON:
        spin_lock_irqsave(&pciDev->irq_lock, flags);
        // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
        // ----- ----- ----- -----0
        cReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->dmaControl);
        cReg.cmdFlds.Invert = 1;
        PCIE_WRITE32(cReg.reg32, (void *)&pciDev->pModDmaHregs->dmaControl);
        // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
        // ----- ----- ----- -----
        spin_unlock_irqrestore(&pciDev->irq_lock, flags);
        ...
        break;
  case ESTHER_TRG_INVERT_OFF:
          spin_lock_irqsave(&pciDev->irq_lock, flags);
          // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
          // ----- ----- ----- -----
          cReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->dmaControl);
          cReg.cmdFlds.Invert = 0;
          PCIE_WRITE32(cReg.reg32, (void *)&pciDev->pModDmaHregs->dmaControl);
          // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
          // ----- ----- ----- -----
          spin_unlock_irqrestore(&pciDev->irq_lock, flags);
    ...
    case ESTHER_TRG_DELAY:

     retval = __get_user(tmp, (int __user *)arg);

            spin_lock_irqsave(&pciDev->irq_lock, flags);
            // ----- ----- DEVICE SPECIFIC CODE ----- -----
              tReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->triggsetting);
              tReg.triggset.delay=tmp;

              PCIE_WRITE32(tReg.reg32, (void *)&pciDev->pModDmaHregs->triggsetting);

            // ----- ----- DEVICE SPECIFIC CODE ----- -----
            spin_unlock_irqrestore(&pciDev->irq_lock, flags);
            break;

     case ESTHER_TRG_RANDOM_ON:
            spin_lock_irqsave(&pciDev->irq_lock, flags);
            // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
            // ----- ----- ----- -----
            tReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->triggsetting);
            tReg.triggset.random = 1;
            PCIE_WRITE32(tReg.reg32, (void *)&pciDev->pModDmaHregs->triggsetting);
            // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
            // ----- ----- ----- -----
            spin_unlock_irqrestore(&pciDev->irq_lock, flags);
            ...
            break;
      case ESTHER_TRG_RANDOM_OFF:
              spin_lock_irqsave(&pciDev->irq_lock, flags);
              // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
              // ----- ----- ----- -----
              tReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->triggsetting);
              tReg.triggset.random = 0;
              PCIE_WRITE32(tReg.reg32, (void *)&pciDev->pModDmaHregs->triggsetting);
              // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
              // ----- ----- ----- -----
              spin_unlock_irqrestore(&pciDev->irq_lock, flags);
          ...
          case ESTHER_TRG_SELECT_ON:
                    spin_lock_irqsave(&pciDev->irq_lock, flags);
                    // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
                    // ----- ----- ----- -----0
                    cReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->dmaControl);
                    cReg.cmdFlds.Select = 1;
                    PCIE_WRITE32(cReg.reg32, (void *)&pciDev->pModDmaHregs->dmaControl);
                    // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
                    // ----- ----- ----- -----
                    spin_unlock_irqrestore(&pciDev->irq_lock, flags);
                    ...
                    break;
          case ESTHER_TRG_SELECT_OFF:
                      spin_lock_irqsave(&pciDev->irq_lock, flags);
                      // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
                      // ----- ----- ----- -----
                      cReg.reg32 = PCIE_READ32((void *)&pciDev->pModDmaHregs->dmaControl);
                      cReg.cmdFlds.Select = 0;
                      PCIE_WRITE32(cReg.reg32, (void *)&pciDev->pModDmaHregs->dmaControl);
                      // ----- ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- -----
                      // ----- ----- ----- -----
                      spin_unlock_irqrestore(&pciDev->irq_lock, flags);
          ...
          case ESTHER_TIMER_GET:
            spin_lock_irqsave(&pciDev->irq_lock, flags);
            //  ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- ----- -----
            tmp = PCIE_READ32((void *)&pciDev->pModDmaHregs->timer_get);
            //  ----- ----- ----- ----- ----- DEVICE SPECIFIC CODE ----- ----- -----
            spin_unlock_irqrestore(&pciDev->irq_lock, flags);
